...
<div class="card card-outline card-danger">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control form-control-md rounded-1" value="<?php echo isset($po_code) ? $po_code : '' ?>" readonly>
                    </div>
                    <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" id="date_created" name="date_created" class="form-control" required 
                        min="<?php echo date('Y-m-01'); ?>" max="<?php echo date('Y-m-t'); ?>">
                </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="department" id="department" class="custom-select select2">
                                <option value=""></option>
                                <option value="pcs">General Admin</option>
                                <option value="pack">Front Office</option>
                                <option value="can">Sales & Marketing</option>
                                <option value="kg">Human Resources</option>
                                <option value="ltr">Housekeeping</option>
                                <option value="can">F&B Product</option>
                                <option value="kg">F&B Service</option>
                                <option value="ltr">Engineering</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row justify-content-center align-items-end">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control rounded-1" id="unit" readonly title="This will be auto-filled when you select the item above">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label for="qty" class="control-label">Qty</label>
                                <input type="number" step="any" class="form-control rounded-1" id="qty">
                            </div>
                        </div>
                        <div class="col-md-2 text-center">
                            <div class="form-group">
                                <button type="button" class="btn btn-md btn-danger" id="add_to_list">Add to List</button>
                            </div>
                        </div>
                </fieldset>
                <hr>
    ...
    <div class="card-footer py-1 text-center">
        <button class="btn btn-md btn-danger" type="submit" form="po-form">Save</button>
        <a class="btn btn-md btn-dark" href="<?php echo base_url.'/admin?page=purchase_order' ?>">Cancel</a>
    </div>
</div>
...
<script>
    ...
    // Definisikan data unit yang telah dimasukkan ke dalam halaman saat pertama kali di-generate
    var unitsData = <?php echo json_encode($unitsDataFromServer); ?>; // Gantilah $unitsDataFromServer dengan data unit dari server

    // Ambil elemen select dan input yang akan digunakan
    var itemSelect = document.getElementById("item_id");
    var unitInput = document.getElementById("unit");

    // Tambahkan event listener untuk peristiwa pemilihan item
    itemSelect.addEventListener("change", function() {
        // Ambil nilai item_id yang dipilih
        var selectedItemId = itemSelect.value;

        // Cari data unit yang sesuai dengan item_id yang dipilih
        var selectedUnit = unitsData.find(function(item) {
            return item.item_id === selectedItemId;
        });

        // Isi nilai unitInput dengan data unit yang sesuai
        if (selectedUnit) {
            unitInput.value = selectedUnit.unit;
        } else {
            unitInput.value = ""; // Jika item tidak memiliki data unit, kosongkan input
        }
    });
</script>